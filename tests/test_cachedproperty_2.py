...
@pytest.mark.parametrize(
    "test_id, initial_value, expected_value",
    [
        ("HP_01", 10, 10),
        ("HP_02", "test", "test"),
        ("HP_03", [1, 2, 3], [1, 2, 3]),
...
def test_cached_property_happy_path(test_id, initial_value, expected_value):
    ...
    ), f"Test ID {test_id}: CachedProperty did not return the expected value."


# Edge case tests
@pytest.mark.parametrize(
    "test_id, initial_value, modify_value, expected_value",
    [
        (
            "EC_01",
            10,
            20,
            10,
        ),  # Changing the underlying value doesn't affect the cached value
    ],
)
def test_cached_property_edge_cases(
    test_id, initial_value, modify_value, expected_value
):
    # Arrange
    test_obj = TestClass(initial_value)
    ...
    assert (
        result == expected_value
    ), f"Test ID {test_id}: CachedProperty did not maintain the initial cached value."


# Error case tests
@pytest.mark.parametrize(
    "test_id, initial_value, expected_exception",
    [
        (
            "ER_01",
            None,
            AttributeError,
        ),  # Accessing the property on None should raise an AttributeError
    ],
)
def test_cached_property_error_cases(test_id, initial_value, expected_exception):
    ...
    with pytest.raises(expected_exception):
        _ = test_obj.value
        pytest.fail(
            f"Test ID {test_id}: CachedProperty should have raised an exception."
        )
...
def test_cached_property_instance_specific():
    ...
    assert (
        result1 != result2
    ), "CachedProperty cache should be instance-specific and not shared between instances."
...
def test_cached_property_access_on_class():
    ...
    ), "Accessing the property on the class should return the CachedProperty instance itself."
